 ...
 *
 */
@Service("AzbjWxxpgService")
public class AzbjWxxpgService extends AbstractService{
	...
	public ResultList<BasicMap<String, Object>> findAll(BasicMap<String, Object> query,Paginate paginate){
		...
		if(user != null){
			String sqlstr = "SELECT a.xm,b.* FROM jz_jzryjbxx AS a INNER JOIN anzbj_wxxpgxxcjb AS b on a.id=b.azbjryid INNER JOIN anzbj_ryxjxxcjb AS c ON a.id=c.id";
	...
	public void saveOne(BasicMap<String, Object> data){
		dbClient.save(SupConst.Collections.ANZBJ_WXXPGXXCJB, data);
	}
	/**
	 * 删除考核内容
	 * @param query
	...
	public void removeOne(String id){
		dbClient.remove(SupConst.Collections.ANZBJ_WXXPGXXCJB, new SqlDbFilter().eq("id",id));
	}
	/**
	 * 查询矫正信息
	 * @return
