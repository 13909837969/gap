...
//sunhailong
@Service("SfsGzrycjpxService")
public class SfsGzrycjpxService extends AbstractService{
		...
		if(user != null){
			...
			list = dbClient.find(adapter, paginate,new RowDataListener() {
				...
				public void processData(BasicMap<String, Object> rowData) {
					rowData.put("pxjsqr", DateUtil.format(rowData.get("pxjsqr"), "yyyy-MM-dd"));
					rowData.put("pxksrq", DateUtil.format(rowData.get("pxksrq"), "yyyy-MM-dd"));
	...
	public void deleteOne(BasicMap<String, Object> query){
		...
		dbClient.remove(SupConst.Collections.SFS_SFSGZRYCJPXQKXXB, new SqlDbFilter().eq("id", StringUtil.toEmptyString(query.get("id"))));
	}
	/**
	 * 查询一条信息
	 * @param query
	...
	public BasicMap<String, Object> findOne(BasicMap<String, Object> query){
		...
		rtn.put("pxjsqr", DateUtil.format(rtn.get("pxjsqr"), "yyyy-MM-dd"));
		rtn.put("pxksrq", DateUtil.format(rtn.get("pxksrq"), "yyyy-MM-dd"));
		return rtn;
	}
