...
//sunhailong
@Service("SfsGzrysccService")
public class SfsGzrysccService extends AbstractService{
		...
		if(user != null){
			...
			list = dbClient.find(adapter, paginate,new RowDataListener() {
				...
				public void processData(BasicMap<String, Object> rowData) {
					rowData.put("cjsj", DateUtil.format(rowData.get("cjsj"), "yyyy-MM-dd"));
	...
	public void deleteOne(BasicMap<String, Object> query){
		...
		dbClient.remove(SupConst.Collections.SFS_SFSGZRYSCCQKXXB, new SqlDbFilter().eq("id", StringUtil.toEmptyString(query.get("id"))));
	}
	/**
	 * 查询一条信息
	 * @param query
	...
	public BasicMap<String, Object> findOne(BasicMap<String, Object> query){
		...
		rtn.put("cjsj", DateUtil.format(rtn.get("cjsj"), "yyyy-MM-dd"));
		return rtn;
	}
