...
//sunhailong
@Service("SfsGzryzwxxService")
public class SfsGzryzwxxService extends AbstractService{
		...
		if(user != null){
			...
			list = dbClient.find(adapter, paginate,new RowDataListener() {
				...
				public void processData(BasicMap<String, Object> rowData) {
					rowData.put("rzkssj", DateUtil.format(rowData.get("rzkssj"), "yyyy-MM-dd"));
					rowData.put("rzjssj", DateUtil.format(rowData.get("rzjssj"), "yyyy-MM-dd"));
	...
	public void saveOne(BasicMap<String, Object> data){
		...
		dbClient.save(SupConst.Collections.SFS_SFSGZRYZWXXB, data);
	}
	/**
	 * 删除一条信息
	 * @param query
	...
	public void deleteOne(BasicMap<String, Object> query){
		...
		dbClient.remove(SupConst.Collections.SFS_SFSGZRYZWXXB, new SqlDbFilter().eq("id", StringUtil.toEmptyString(query.get("id"))));
	}
	/**
	 * 查询一条信息
	 * @param query
	...
	public BasicMap<String, Object> findOne(BasicMap<String, Object> query){
		...
		rtn.put("rzkssj", DateUtil.format(rtn.get("rzkssj"), "yyyy-MM-dd"));
		rtn.put("rzjssj", DateUtil.format(rtn.get("rzjssj"), "yyyy-MM-dd"));
		return rtn;
	}
