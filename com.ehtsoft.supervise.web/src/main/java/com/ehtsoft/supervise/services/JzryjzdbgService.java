 ...
 */
@Service("JzryjzdbgService")
public class JzryjzdbgService extends AbstractService {
	...
	public ResultList<BasicMap<String, Object>> find(BasicMap<String, Object> data, Paginate paginate) {
		...
		/*String sqlstr ="select distinct a.audit,b.f_id,c.*,"
				...
				+ "inner join JZ_JZDBGXXCJB c on c.id = b.f_bsid "
				+ "inner join JZ_JZRYJBXX d on  d.id = c.f_aid ";  */
		String sqlstr="select a.*,b.id,b.xm,b.sqjzrybh as jzrybh from JZ_JZDBGXXCJB a inner join JZ_JZRYJBXX b on b.id = a.f_aid";
	...
	public void save(BasicMap<String, Object> data) {
		...
		if(f_aid==null){
			throw new AppException("居住地变更的服刑人员不能为空");
		}
		...
		auditApply.setTitle(fxryxm + "的居住地变更的审批上报");
	...
	//保存审批
		...
		public BasicMap<String, Object> findOne(String id) {
			String sqlstr = "select b.xm,b.xb,b.sfzh,b.sqjzrybh,c.jzlb,c.sqjzqx,c.jzsssq,c.sqjzksrq,c.sqjzjsrq,c.jtzm,c.ypxq,a.*,"
					+ "d.hjszdmx,d.gdjzdmx " + "FROM JZ_JZDBGXXCJB a "
