<%@ page pageEncoding="utf-8" contentType="text/html;charset=utf-8"%>

<html>
<head>
<title>司法所业务用房信息管理</title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<jsp:include page="../ltrhao-common.jsp"></jsp:include>
<script type="text/javascript" src="${localCtx}/json/JzDcpgxxService.js"></script>
<script type="text/javascript">
 	
	$(function() {
		var form = new Eht.Form({
			selector : '#sqjz_fromDcpg_add_form' 
		});
		$("#sqjz_listDcpg_all #btn-primary").click(function() {
			console.log(form.getData());
			var service = new JzDcpgxxService();
			service.save(form.getData(), new Eht.Responder({
				success : function() {
					$("#tableaaa").refreshTable();
					$('#sqjz_listDcpg_all #myModal').modal('hide');
				}
			}));
		});
	});
	$("#sqjz_listDagl_all #quxiaobtnCj").click(function(){//单击取消按钮
		$("#sqjz_listDagl_all #hideDivCj").hide();
	}); 
</script>
<style type="text/css">
	.div span{
		position:absolute;
		right:3%;
		display:inline;
		font-size: 15px;
		margin-top: 5px;
	}
	.div input{
		display:inline;
	}
	.ds{
		width: 800px;
		margin-left: 260px;
	}
	
</style>
</head>
<body>
<div class="ds" >
	<h3 align="center">司法所业务用房信息表</h3><br/>
	<div id="sqjz_fromDcpg_add " >
		<div id="sqjz_fromDcpg_add_form">
			<div class="row form-group">
				<label for="firstname" class="col-sm-2 control-label">司法所编码：</label>
				<div class="col-sm-10">
					<input name="sfsbm" type="text" class="form-control"
						id="firstname" placeholder="司法所编码">
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">独立业务用房:</label>
				<div class="col-sm-10">
					<input name="dlywyf" type="radio" id="lastname" checked="checked">是
					<input name="dlywyf" type="radio" id="lastname">否
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">办公场所实景:</label>
				<div class="col-sm-10">
					<select name="bgcssj" class="form-control" id="lastname" placeholder="办公场所实景"> 
						<option value=""></option> 
						<option value="01">司法所照片</option>
						<option value="02">办公场所实景</option>
						<option value="03">制度文件</option>
						
					</select>
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">业务用房产权归属：</label>
				<div class="col-sm-10">
						<select name="ywyfcqgs" class="form-control" id="lastname" placeholder="业务用房产权归属"> 
						<option value=""></option> 
						<option value="01">县（市、区）司法局</option>
						<option value="02">乡镇（街道）政府</option>
						<option value="03">司法所</option>
						<option value="99">其他</option>
						
					</select>
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">业务用房投资方：</label>
				<div class="col-sm-10">
					<select name="ywyftzf" class="form-control" id="lastname" placeholder="业务用房投资方"> 
						<option value=""></option> 
						<option value="01">中央投资</option>
						<option value="02">地方投资</option>
						<option value="03">中央和地方共同投资</option>
						<option value="99">其他</option>
					</select>
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">业务用房性质：</label>
				<div class="col-sm-10">
					<select name="ywyftzf" class="form-control" id="lastname" > 
						<option value=""></option> 
						<option value="01">划拨</option>
						<option value="02">租赁</option>
						<option value="03">自有</option>
						<option value="99">其他</option>
					</select>
				</div>
			</div>
			
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">业务用房面积：</label>
				<div class="col-sm-10 div" >
					<input name="ywyfmj" type="text" valid="{number:true}" class="form-control" id="lastname"
						placeholder="业务用房面积" ><span>㎡</span>
				</div>
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">业务用房投资金额：</label>
				<div class="col-sm-10 div" >
					<input name="ywyfmj" type="text" class="form-control" id="lastname"
					placeholder="业务用房投资金额"  ><span>￥</span>
				</div>
			</div>
			
			</div>
			
			</div>
			<div class="row form-group">
				<label for="lastname" class="col-sm-2 control-label">建成日期：</label>
				<div class="col-sm-10">
					<div class="input-group input-group-sm">
					    <input name="jcrq" type="text" class="form-control form_date" data-date-format="yyyy-MM-dd" id="lastname">
					    <span class="input-group-addon calendar"><span class="glyphicon glyphicon-calendar"></span></span>
				   </div>
				</div>
			</div>
			<div class="row form-group"  align="right">
				<button type="button" class="btn btn-default quxiaobtnCj sqjz_listDcpg_all" data-dismiss="modal">关闭</button>
							<button id="dagl_list_btn_primary" type="button" class="btn btn-primary sqjz_listDcpg_all">提交</button>
			</div>
			
		</div>
	</div>
</div>
</body>
</html>