...
public class MessageEhtListenerServlet extends MessageServletSupport {
    private static final Logger LOG = LoggerFactory.getLogger(MessageEhtListenerServlet.class);
    ...
    protected AjaxWebClient getAjaxWebClient( HttpServletRequest request ) {
        ...
        // 将 客户端 添加到 Session 中，用于 在 session 失效的时候，将其关闭 
        Map<String,AjaxWebClient> m=(Map<String,AjaxWebClient>)request.getSession().getAttribute("hap.ajax.webclients");
        if(m==null){
        	m = new HashMap<String,AjaxWebClient>();
        	m.put(sessionKey,client);
        	request.getSession().setAttribute("hap.ajax.webclients", m);
        }else{
        	m.put(sessionKey, client);
        }
