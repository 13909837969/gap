...
//sunhailong
@Service("SqjzzxService")
public class SqjzzxService extends AbstractService{
	...
	public ResultList<BasicMap<String, Object>> findAll(BasicMap<String, Object> query,Paginate paginate){
		...
		if(user != null){
			...
			String sql = "select * from JZ_SQJZZX";
	...
	public void saveOne(BasicMap<String, Object> data){
		dbClient.save(SupConst.Collections.JZ_SQJZZX, data);
	}
	/**
	 * 删除一条信息
	 * @param query
	...
	public void deleteOne(BasicMap<String, Object> query){
		dbClient.remove(SupConst.Collections.JZ_SQJZZX, new SqlDbFilter().eq("id", StringUtil.toEmptyString(query.get("id"))));
	}
	/**
	 * 查询一条信息
	 * @param query
	...
	public BasicMap<String, Object> findOne(BasicMap<String, Object> query){
		...
		rtn = dbClient.findOne(SupConst.Collections.JZ_SQJZZX, new SqlDbFilter().eq("id", StringUtil.toEmptyString(query.get("id"))));
		return rtn;
	}
}
