 ...
 * 矫正人员个人基本信息表 + 外出申请信息
 * @author   :dd
 * @创建时间 :2017年11月26日 上午11:11:26
 * @修改人	 :
 * @创建时间 :2017年11月26日
 */
@Service("JzryWcspService")
public class JzryWcspService  extends AbstractService  {
	 ...
	 * 矫正人员个人基本信息表 + 外出申请信息
	 * 根据	
	 * 		个人基本信息表内的ID=外出申请信息采集表内的F_AID	获取外出申请信息
	 * @param id
	 * @return
	 */
	public BasicMap<String, Object> findOne(String id){
		...
		/**
		 * 根据	个人基本信息表内的ID=外出申请信息采集表内的F_AID	获取外出申请信息
		 */
		List<BasicMap<String,Object>> wcsq = dbClient.find(SupConst.Collections.JZ_WCSQXXCJB, new SqlDbFilter().eq("f_aid", id),new RowDataListener(){
			public void processData(BasicMap<String, Object> rowData) {
				/**
				 * 处理表内相关字段的时间格式
				 */
				rowData.put("sqsj",  DateUtil.format(rowData.get("sqsj"),"yyyy-MM-dd HH:mm"));
				rowData.put("ksqr",  DateUtil.format(rowData.get("ksqr"),"yyyy-MM-dd HH:mm"));
				rowData.put("jsrq",  DateUtil.format(rowData.get("jsrq"),"yyyy-MM-dd HH:mm"));
	 ...
	 * 根据	
	 * 		个人基本信息表内的【XM】模糊查询
	 * 			个人信息表与外出申请信息采集表关联查询得到的结果	获取外出申请信息
	 * @param id
	 * @return
	 */
	public List<BasicMap<String, Object>> findName(String name, Paginate paginate){
		String sql = "select b.xm,b.sfzh,b.grlxdh,a.* from JZ_WCSQXXCJB a,JZ_JZRYJBXX b WHERE a.f_aid=b.id and xm like '%" +name+ "%'";
	...
	public List<BasicMap<String, Object>> findAll(BasicMap<String, Object> query, Paginate paginate){
		...
		String sql = "select b.xm,b.sfzh,b.grlxdh,a.*  from JZ_WCSQXXCJB a,JZ_JZRYJBXX b WHERE a.f_aid=b.id order by a.sqsj";
		SQLAdapter sqlAdapter = new SQLAdapter(sql);
