 ...
 * @author wangbao
 */
public class SentVideoSocketHandler extends AbstractWebSocketHandler{
	private Log logger = LogFactory.getLog(SentVideoSocketHandler.class);
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		try{
			//将当前连接加载到缓存中
			VideoSocketContext.addSendSession(fromAid, session);
		}catch(Exception ex){
	...
	protected void handleBinaryMessage(WebSocketSession session, BinaryMessage message) throws Exception {
		try{
			String toAid = VideoSocketContext.getToAid(session);
				if(toAid!=null){
					WebSocketSession toSession = VideoSocketContext.getSendSessionClient(toAid);
					try{
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		try{
			VideoSocketContext.removeSendSession(session);
		}catch(Exception ex){
			logger.error(ex.getMessage(),ex);
		}
	}
