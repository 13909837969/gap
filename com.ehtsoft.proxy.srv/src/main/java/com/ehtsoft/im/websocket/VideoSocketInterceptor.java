...
public class VideoSocketInterceptor  extends HttpSessionHandshakeInterceptor{
	private Log logger = LogFactory.getLog(VideoSocketInterceptor.class);
	/**
	...
	public boolean beforeHandshake(ServerHttpRequest serverHttpRequest, ServerHttpResponse serverHttpResponse, WebSocketHandler wsHandler, Map<String,Object> attributes) throws Exception {
		...
		String toAid = request.getParameter(VideoSocketContext.TO_ACCOUNT_ID);
		String fromAid = request.getParameter(VideoSocketContext.FROM_ACCOUNT_ID);
		
		attributes.put(VideoSocketContext.TO_ACCOUNT_ID, toAid);
		attributes.put(VideoSocketContext.FROM_ACCOUNT_ID, fromAid);
