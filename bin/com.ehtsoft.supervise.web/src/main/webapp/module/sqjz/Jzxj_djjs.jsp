	...
	<script type="text/javascript">
		$(function(){
			...
			table.clickRow(function(data){
				...
				changeInput(false);
				if(data.sfzp== 1){
					$("#sqjzdagl_jbxxZj #btnAddZp").attr("disabled",true);
				}
			});
			//隐藏的警告框触发事件
			...
			/* 单击查询按钮触发事件 */
			$("#sqjz_listDagl_all #search").click(function(){
			...
			/* 单击新增按钮触发事件 */
			$("#sqjz_listDagl_all #djjs").click(function(data){
				$("#formSimple-body #photo_remark").removeAttr("display");
				$("#sqjz_listDagl_all #myModalAdd").modal({backdrop: 'static', keyboard: false});//弹出模态框
				$("#sqjz_listDagl_all #modal-bodyAdd").load("${localCtx}/module/sqjz/formDagl.jsp?load=load");
			});
			...
			/* 单击修改按钮，修改选中信息 */
		   	...
		   	$("#sqjz_listDagl_all #btnUp").click(function(){
		   		if(json.id==null){
				  	...
				  	$("#sqjz_listDagl_all #modal-bodyAdd").load("${localCtx}/module/sqjz/formDagl.jsp?load=load&id="+json.id);
		  ...
		  //点击虚拟身份按钮时间  完成
		    ...
		    $("#btnAddGljb").click(function(){
		    	$("#sqjz_listDagl_all #myModalLabel").html("管理级别信息");
		   		$("#sqjz_listDagl_all #selectMyModal").modal({backdrop: 'static', keyboard: false});//弹出模态框
		   		$("#sqjz_listDagl_all #select-modal-body").load("${localCtx}/module/sqjz/listGljbxx.jsp?load=load&id="+json.id,function(){
		   			$().initDatetimepicker();
		   		});
			}); 
