$(function() {
	...
	var sum2_fx="";
	var sum3_azbj="";
	var data_nl_dw = new Array(); //数组对象[定位信息] 
	...
	r.findAllsl(new Eht.Responder({success:function(data){
		...
		//加载地图数据
			...
			r.findDtData(new Eht.Responder({success:function(data){
				 ...
				 var falg = true;
				 refresh(data_nl_dw,data_nl,falg);
			}}));
}}));
	 var r = new Rep_Rmtj_Test();
	 //监狱服刑人员
	 ...
	 function initZb(){
		 var type = "15";
		 var data_nl = new Array(); //数组对象 
		 r.findJyzs(type,new Eht.Responder({success:function(data){
			 for (var i = 0; i < data.length; i++) {
	...
	//安置帮教人员
	 ...
	 initZb();
	 $("#ltrhao-body-left2-div1").css('background-color', '#666');
	
//刷新界面
function refresh(datadw,datas,falg) {
	 //随机数处理
