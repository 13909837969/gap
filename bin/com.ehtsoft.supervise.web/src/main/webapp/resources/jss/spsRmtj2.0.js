...
function qqrmtjgzzl() {
	...
	r.findQyrmtjzlzl(new Eht.Responder({success:function(data){
		//全区人民调解工作总览数据
		var table = $("#rmtjTable"); 
		for (var i = 0; i < data.length; i++) {
			   ...
			   var  tr = $('<tr height="20px" id="'+i+'"class="city-hight" style="color:#fff;">'
						+	'<td width="30px;" class="city-hight"><span id="spanId" class="badge" style="width:35px;(i<=3 ? "background:#f00;" : "background:#333;")">'+(i+1)+'</span></td>'
			      		+	'<td align="center"  width="100px;" class="city-hight">'+data[i].msmc+'</td>'
			      		+	'<td align="center"  width="100px;" class="city-hight">'+data[i].sl1+'件</td>'
			      		+   '<td align="center"  width="50px;" class="city-hight">'+data[i].zb+'%</td>'
			      		+	'<td width="50px;" style="padding-left:50px;" class="city-hight"><span class="badge" style="width:70px;">'+data[i].tbzz+'%</span></td>'
			      		+	'</tr><hr/>');
				table.append(tr);
