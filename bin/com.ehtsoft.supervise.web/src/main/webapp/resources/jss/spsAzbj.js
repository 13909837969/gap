	$(function(){
		...
		//显示隐藏的区域选择
		$("#spsAzbj-container-left1-diva").click(function(){
				$("#spsAzbj-hidenDiv").css("height","0").show().animate({height:400});
			return false;
		});
		//点击其他地方区域隐藏
		$(window).click(function(){
			$("#spsAzbj-hidenDiv").animate({height:0},function(){$(this).hide()});
		});

		$("#spsAzbj-hidenDiv").click(function(){return false;});
		...
		service.find({},new Eht.Responder({
			success:function(data){
				for(var i=0;i<data[0].nodes.length;i++){
					var table = $("#azbjTable"); 
					var tr = $('<tr height="30px" style="margin-left:10px;" id="'+i+'">'
							+	'<td width="90px;"><span class="badge"  style="background-color:#4782fa;">'+(i+1)+'</span></td>'
				      		+	'<td align="center"  width="100px;">'+data[0].nodes[i].region_name+'</td>'
				      		+	'<td align="center"  width="140px;" style="padding-left:10px;">2330人</td>'
			      		+	'</tr>');
					table.append(tr);
				}
			}
